...
/** Role card — the ward as dots; one of them keeps turning. */
export function ArtWard() {
  return (
      ...
      {Array.from({ length: 10 }).map((_, i) => {
        ...
        return (
          <motion.span
            ...
            style={{ background: hot ? undefined : "var(--teal)" }}
            animate={
              hot
                ? { backgroundColor: ["#2dd4bf", "#fbbf24", "#fb923c", "#f87171", "#2dd4bf"], scale: [1, 1.05, 1.12, 1.2, 1] }
                : { opacity: [0.18, 0.35, 0.18] }
            }
            transition={hot ? { duration: 6, repeat: Infinity, ease: "easeInOut" } : { duration: 3 + (i % 3), repeat: Infinity, delay: i * 0.2 }}
          />
        );
      })}
    </div>
...
/** Role card — calm breathing rings around a plain-language status. */
export function ArtBreath() {
  return (
    <div className="relative grid size-40 place-items-center">
      {[0, 1, 2].map((i) => (
        <motion.span
          ...
          animate={{ scale: [0.55, 1.05], opacity: [0.7, 0] }}
          transition={{ duration: 4, repeat: Infinity, delay: i * 1.33, ease: "easeOut" }}
        />
      ))}
      <span className="text-[22px] font-medium text-teal" lang="hi">आप ठीक हैं</span>
    </div>
  );
}
